...
module Garden =
    ...
    let item (x, y) (garden: Garden) =
        garden[y, x]

    let itemOrFallback (x, y) fallback (garden: Garden) =
        if x >= 0 && x < Array2D.length2 garden && y >= 0 && y < Array2D.length1 garden then
            garden[y, x]
        else
            fallback
...
let findArea plant (sx, sy) (garden: Garden) =
    let folder (x, y) (queue, area) (dx, dy) =
        ...
        if Garden.isInRange (nx, ny) garden
            ...
            && Garden.item (nx, ny) garden = plant
        then
...
let (|Even|Uneven|) (p1, p2) =
    if p1 = p2 then Even else Uneven
...
let findDistinctSides (before1, before2, positions) (current1, current2) =
...
let countSides f (garden: Garden) =
    let count = function | Some x -> Some (x + 1) | None -> Some 1
            ...
            |> List.sumBy (fun positions ->
                ...
                let sides =
                    ...
                    |> Seq.sumBy (fun pos ->
                        ...
                        Map.tryFind key numberOfSides |> Option.defaultValue 0)
                
                area * sides))

    totalPrice

let partOne (garden: Garden) =
    solve perimeter garden
    
let partTwo (garden: Garden) =
    solve findDistinctSides garden
